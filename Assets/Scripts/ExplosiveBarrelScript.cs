	...
	private IEnumerator Explode () {
		...
		foreach (Collider hit in colliders) {
			...
			if (hit.transform.tag == "Zombie") 
			{
				...
				hit.transform.gameObject.GetComponentInParent<Enemy>().DamageEnemy(explosionDamage);
			}

            if (hit.transform.tag == "GameController")
            {
                //Toggle the isHit bool on the target object
                hit.transform.gameObject.GetComponent<PlayerLife>().DamagePlayer(explosionDamage);
            }
